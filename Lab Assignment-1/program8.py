...
j=0    
for _ in range(len(cipher_text)//2):
...
for i in divide_message:
    ...
    if a1==a2:
            ...
            if b2==4:
                    b2=-1
            plain_text.append(key_matrix[a1][b1-1])
            plain_text.append(key_matrix[a1][b2-1])
    elif b1==b2:
            if a1==4:
                    a1=-1;
            if a2==4:
                    a2=-1;
            plain_text.append(key_matrix[a1-1][b1])
            plain_text.append(key_matrix[a2-1][b2])
    else:
           plain_text.append(key_matrix[a1][b2])
           plain_text.append(key_matrix[a2][b1])
str1=''.join(plain_text)
my_list=list(str1)
a='X'
for i in my_list:
    if a in i:
